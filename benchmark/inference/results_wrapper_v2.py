...
def plot_grid(data):
    ...
    fig.update_layout(height=600, width=1800, title_text=" 2xICX + 512GB RAM, Model = gcn, Dataset = ogbn_products")
    fig.update_xaxes(type='category', categoryarray=np.unique(data["NR_WORK"]))
    fig.update_yaxes(dtick=10)            
    fig.write_image(f"{PLOTS}/plot.png")
    
         
def model_mask(data):
    data['setup'] = np.nan
    data['COMP_AFFINITY'] = data['COMP_AFFINITY'].astype(int)
    data['DL_AFFINITY'] = data['DL_AFFINITY'].astype(int)
...
if __name__ == '__main__':
    ...
    baseline_data = baseline_data.loc[(baseline_data['ST'] == 'True') & (baseline_data['HYPERTHREADING'] == '1') & (baseline_data['H'] == '128')]
    aff_data = affinity_data.loc[affinity_data['HYPERTHREADING'] == '1']
    
    data = pd.concat([baseline_data, aff_data])
